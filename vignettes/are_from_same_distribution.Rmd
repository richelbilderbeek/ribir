...
df_new <- df %>% group_by(treatment) %>% summarise(p_value = wilcox.test(d1, d2)$p.value)
...
df <- data.frame(
  ...
  i = rep(1:5, times = 2), # You will get 'Duplicate identifiers for rows' if you remove this
  x = c(runif(5), rnorm(5)),
...
df_new <- df %>% group_by(treatment) %>% summarise(p_value = wilcox.test(B, C)$p.value)
